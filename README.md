...
* [`src/github_repo_tracker/`](src/github_repo_tracker/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/github_repo_tracker/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `github-repo-tracker` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/github_repo_tracker/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns github-repo-tracker.example
  (:require [github-repo-tracker.config :as config])
